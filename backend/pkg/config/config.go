...
func addGeneralFlags(f *flag.FlagSet) {
	...
	f.Bool("enable-cluster-inventory", false, "Enable automatic discovery of clusters from ClusterProfile resources (Cluster Inventory API)")
	f.String("cluster-inventory-provider-file", "", "Path to the JSON configuration file for ClusterProfile access providers (exec plugins)")
