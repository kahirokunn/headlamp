...
the clusters that consumer needs. For example, an Open Cluster Management
integration can bind only the ManagedClusterSets Headlamp should consume into
Headlamp's namespace with `ManagedClusterSetBinding` resources, then publish
the corresponding ClusterProfiles there.
